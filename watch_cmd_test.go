...
func TestCmdWatchAddCheckCycle(t *testing.T) {
	...
	if err := cmdWatch(ctx, st, []string{"add", "Sol", "Ring", "--under", "5"}, false); err != nil {
	...
	if err := cmdWatch(ctx, st, nil, false); !errors.Is(err, errWatchFired) {
		t.Fatalf("first check = %v, want errWatchFired", err)
	}
	if err := cmdWatch(ctx, st, nil, false); err != nil {
...
func TestCmdWatchAddRejectsBadFlags(t *testing.T) {
	...
	for _, args := range [][]string{
		...
		if err := cmdWatch(ctx, st, args, false); err == nil {
			t.Errorf("cmdWatch(%v) succeeded, want an error", args)
		}
	}
	// Subcommands reject --json rather than printing a table at a script.
	if err := cmdWatch(ctx, st, []string{"list"}, true); err == nil {
...
func TestCmdWatchRemoveByFragment(t *testing.T) {
	...
	stubFetch(t, watchCard())
	ctx := context.Background()
	if err := cmdWatch(ctx, st, []string{"add", "Sol Ring", "--over", "30"}, false); err != nil {
		t.Fatalf("watch add: %v", err)
	}
	if err := cmdWatch(ctx, st, []string{"rm", "sol"}, false); err != nil {
...
func TestCmdWatchFoil(t *testing.T) {
	...
	stubFetch(t, watchCard())
	ctx := context.Background()
	if err := cmdWatch(ctx, st, []string{"add", "Sol Ring", "--foil", "--over", "10"}, false); err != nil {
		t.Fatalf("watch add: %v", err)
	}
...
func TestCmdWatchAddUnknownCard(t *testing.T) {
	st := exportStore(t)
	stubFetch(t, watchCard())
	err := cmdWatch(context.Background(), st, []string{"add", "No Such Card", "--under", "5"}, false)
