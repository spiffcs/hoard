...
func TestCmdWatchImportEndToEnd(t *testing.T) {
	...
	if err := cmdWatch(ctx, st, []string{"import", csvPath}, false); !errors.Is(err, errPartial) {
	...
	if err := cmdWatch(ctx, st, []string{"import", jsonPath}, false); err != nil {
...
func TestCmdWatchImportArgErrors(t *testing.T) {
	st := exportStore(t)
	ctx := context.Background()
	if err := cmdWatch(ctx, st, []string{"import"}, false); err == nil {
		t.Error("import without a file must error")
	}
	if err := cmdWatch(ctx, st, []string{"import", "a.csv", "b.csv"}, false); err == nil {
		t.Error("import with two files must error")
	}
	if err := cmdWatch(ctx, st, []string{"import", filepath.Join(t.TempDir(), "absent.csv")}, false); err == nil {
