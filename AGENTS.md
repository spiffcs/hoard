...
Notes for coding agents, and for anyone who has not worked in this repository
before. Everything here is a rule that is easy to break without noticing —
...
sources. Nothing checks them — a stale one is simply committed. Run
`make asset-review`, which rebuilds them and reports which are older than their
source. Images are judged by eye or not at all.
...
**`--json` is refused where it is not honoured.** A command that silently ignored
the flag would be indistinguishable, to a parser, from one that respected it.
Commands opt in with `cli.JSONCapable`; adding JSON output means adding both.
