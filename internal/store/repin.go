...
func (s *Store) RepointDeckPrintings(containerID int64, mapping map[string]string) (int, error) {
	...
	for from, to := range mapping {
		rows, err := tx.Query(`
SELECT finish, condition, board, purchase_price, quantity FROM card_entries
WHERE container_id = ? AND scryfall_id = ?`, containerID, from)
		...
		for rows.Next() {
			var r entryRow
			if err := rows.Scan(&r.finish, &r.condition, &r.board, &r.paid, &r.qty); err != nil {
				rows.Close()
				return 0, err
			}
		...
		for _, r := range moves {
			if _, err := tx.Exec(entryAccumulateSQL,
				containerID, to, r.finish, r.condition, r.board, r.paid, r.qty); err != nil {
