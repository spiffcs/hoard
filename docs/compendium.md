...
`hoard` normally opens your collection. `hoard compendium` builds a different
kind of database called a compendium.
...
| `--era` | Narrow `--format` to its own era — a set list for `premodern`, a release-date cutoff for `predh`. Refused for formats with no era. An explicit `--sets` wins over it. |
...
the one exception — Scryfall varies it by printing, so the Alpha Serra Angel is
...
Two formats carry an era today, and they are bounded differently:
...
The shapes differ because the formats do. PreDH is Commander with a date
cutoff, and Commander takes every paper set, so a release date says it exactly
— Portal and the Starter sets are in. Premodern covers a similar window but
curates those same sets *out*, so only an explicit list is right for it.
...
Standard rotates. `--era` on those is an error rather than a silent no-op, so a
