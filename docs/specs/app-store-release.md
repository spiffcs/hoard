...
| **App icon** | `icon-1024.png` is a true 1024×1024 file, generated by `make-icon.swift` from `docs/assets/hoard-logo.png` — the same artwork the README shows | **Resolved 2026-08-11.** The `.icns` this used to crop is gone, and with it the ~3× upscale: the logo's 645×485 of ink is drawn at 798×600, so ~1.2×. Verified `hasAlpha: no`, which is the property that gets a submission rejected |
...
- ~~**App icon** is upscaled ~3× from a 356px crop.~~ **Done 2026-08-11.** The
  icon is now built from `docs/assets/hoard-logo.png`, the same artwork the
  README shows, at ~1.2× rather than ~3×. `make-icon.swift` composites it
  full-bleed on an opaque field; the `.icns` it used to read no longer exists.
