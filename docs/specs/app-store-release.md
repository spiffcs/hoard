...
| **App icon** | `icon-1024.png` is a true 1024×1024 file, `hasAlpha: no` — the property that gets a submission rejected | **Resolved 2026-08-11, source changed 2026-08-12.** It is no longer generated: the icon is now hand-authored full-bleed artwork committed directly, and `make-icon.swift` has been deleted. That generator found the mark by its alpha bounding box, so an opaque edge-to-edge icon came back shrunk to 82% and letterboxed (measured). The size and alpha guarantees are unchanged and re-verified; what is gone is the guarantee that the phone and the README wear the same face |
...
- ~~**App icon** is upscaled ~3× from a 356px crop.~~ **Done 2026-08-11,
  superseded 2026-08-12.** The upscale is moot: the icon is no longer derived
  from anything. It is hand-authored 1024×1024 artwork committed directly, and
  `make-icon.swift` is deleted — a finished icon is opaque edge to edge, and
  that script located the mark by its alpha bounding box, so it shrank the
  whole composition to 82% and letterboxed it. Replacing the icon is now a
  file copy; the two invariants to preserve by hand are 1024×1024 and no alpha.
