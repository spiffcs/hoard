...
| catalog update | `action.CatalogUpdate` | `catalog update` | palette | downloading catalog (d, bytes) | — | `CatalogUpdateResult` | 0/1 | migrated |
| ensure catalog | `action.EnsureCatalog` | (inside update-prices) | confirm modal (channel bridge; any key but y declines) | downloading catalog (d, bytes) | build?/refresh? | usable bool | — | migrated |
| update prices | `action.UpdatePrices` | `update-prices` | palette | checking catalog (i) · downloading catalog (d) · refreshing cards (d, cards) · saving (i) · filling price gaps (i) · recording history (i) | via ensure | `UpdatePricesResult` | 0/1 | migrated |
| gap fill | `action.FillGaps` | (inside update-prices, import, adds) | — | filling price gaps (i, notes) | — | `pricing.GapReport` | — | migrated |
| backfill prices | `action.BackfillPrices` | `backfill-prices` | palette / `F` on movers | downloading history (d, bytes) · recording (d, rows) | — | `BackfillResult` | 0/1 | migrated |
| import | `action.ImportCollection` | `import` | palette (Import…; re-import confirm; result overlay) | resolving cards (d, cards) | — | `ImportResult` | 0/1/2 | migrated |
...
| movers | `Deps.Movers` | `movers` | `v` view, `W` window | — | — | changes | 0/1 | migrated |
| unpriced | `Deps.Unpriced` | `unpriced` | `v` view | — | — | rows | 0/1 | migrated |
| binder new/rename/rm | store-direct (by design) | `binder …` | `n`/`R`/`d` | — | — | — | 0/1 | by design |
| browse/filter/edit | — (TUI-native) | reverse-skew backlog: `hoard search`, quantity set/rm | `/`, `+/-`, `d`, `u` | — | — | — | — | documented gap |

The TUI has no exit codes; `ErrPartial` outcomes surface as a
success-styled status line with the skip count ("· N skipped") and the
specifics in the result overlay — the work committed either way.
